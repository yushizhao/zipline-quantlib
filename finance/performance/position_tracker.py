...
class PositionTracker(object):
    ...
    def maybe_create_exercise_transaction(self, asset, dt, data_portal):
        if not self.positions.get(asset):
            return None

        amount = self.positions.get(asset).amount
        underlying_price = data_portal.get_spot_value(
            asset, 'close', dt, self.data_frequency)

        # Get the last traded price if price is no longer available
        # if isnan(underlying_price):
        #     underlying_price = data_portal.get_spot_value(
        #     asset, 'close', dt, self.data_frequency)
        
        if asset.callput == 'C':
            price = underlying_price - asset.strike
            if price < 0:
                price = 0

        else:
            price = asset.strike - underlying_price
            if price < 0:
                price = 0
    ...
    # yuusha note: use mid price here    
    def sync_last_sale_prices(self, dt, handle_non_market_minutes,
        ...
        if not handle_non_market_minutes:
            for asset, position in iteritems(self.positions):
                ...
                last_sale_ask = data_portal.get_spot_value(
                    asset, 'ask', dt, self.data_frequency
                )

                last_sale_bid = data_portal.get_spot_value(
                    asset, 'bid', dt, self.data_frequency
                )

                last_sale_price = (last_sale_ask + last_sale_bid)/2

                if not np.isnan(last_sale_price):
                    position.last_sale_price = last_sale_price
        else:
            for asset, position in iteritems(self.positions):
                last_sale_ask = data_portal.get_adjusted_value(
                    asset,
                    'ask',
                ...
                last_sale_bid = data_portal.get_adjusted_value(
                    asset,
                    'bid',
                    data_portal.trading_calendar.previous_minute(dt),
                    dt,
