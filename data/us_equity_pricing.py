...
OHLC = frozenset(['adjusted_close', 'ask', 'bid', 'close'])
US_EQUITY_PRICING_BCOLZ_COLUMNS = (
    'open_interest', 'adjusted_close', 'ask', 'bid', 'close', 'volume', 'day', 'id'
)
...
def to_ctable(raw_data, invalid_data_behavior):
    ...
    if 'open_interest' in raw_data.columns:
        processed['open_interest'] = raw_data.open_interest.astype('uint32')
    else:
        processed['open_interest'] = int(1e9)    
...
class BcolzDailyBarReader(SessionBarReader):
    ...
    def get_value(self, sid, day, colname):
        ...
        if price == 0:
            return -1
        if colname not in ('volume', 'open_interest'):
            return price * 0.001
        else:
            return price
...
class PanelBarReader(SessionBarReader):
    ...
    def __init__(self, trading_calendar, panel, data_frequency):
        ...
        if 'open_interest' not in panel.minor_axis:
            # Fake place_holder if it does not exist.
            panel.loc[:, :, 'open_interest'] = int(1e9)
