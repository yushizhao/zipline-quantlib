...
class AlgorithmSimulator(object):
    ...
    def transform(self):
        ...
        def once_a_day(midnight_dt, current_data=self.current_data,
            ...
            self._exercise_matured_assets(midnight_dt, position_assets)
    ...
    def _exercise_matured_assets(self, dt, position_assets):
        """
        Exercise any assets that have matured before starting a new sim day.

        Performs two functions:

        1. Finds all assets for which we have open orders and clears any
           orders whose assets are on or after their expiration.
        ...
        def past_expiration(asset):
            exp = asset.expiration
            import pandas as pd
            return exp is not None and pd.to_datetime(exp, utc=True) <= dt

        # Remove positions in any sids that have reached their auto_close date.
        assets_to_clear = \
            [asset for asset in position_assets if past_expiration(asset)]
        perf_tracker = algo.perf_tracker
        data_portal = self.data_portal
        for asset in assets_to_clear:
            perf_tracker.process_exercise(asset, dt, data_portal)
